<template>
    <div class="flex flex-col">
        ...
        <div  class="flex-auto flex flex-col rounded-lg bg-slate-700">
            <div class="m-2 w-auto rounded-lg bg-slate-800 h-32 border-4 flex flex-col text-white p-2 border-none justify-center" v-for="task in props.tasks" :key="task.id">
                <input @change="emits('changeTask', {
                    ...task,
                    name: $event.target.value
                })" :value="task.name" class="bg-transparent h-8 font-semibold rounded-lg focus-visible:border-none" id=""  />
                <div @click="emits('selectTask', task)" class="flex-auto text-sm text-gray-500 font-semibold">{{task.subTasks.filter(s => s.checked).length}} of {{task.subTasks.length}} subtasks </div>
            </div>
