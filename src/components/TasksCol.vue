<template>
    <div class="flex flex-col">
        <div class="text-white text-sm text-center justify-center items-center uppercase p-3 font-semibold">
        ...
        <div class="flex-auto flex flex-col rounded-lg bg-slate-700">
            <div class="m-2 w-auto rounded-lg bg-slate-800 h-32 border-4 flex text-white p-2 border-none" v-for="task in props.tasks" :key="task.id">
                <textarea :value="task.name" class="bg-transparent" id="" cols="30" rows="10"></textarea>
            </div>
...
const emits = defineEmits(['setName', 'setTask'])
